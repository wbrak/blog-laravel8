            ...
            <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead>
                        <tr>
                            <th class="px-6 py-3 bg-gray-50 text-center text-base leading-4 font-medium text-gray-500 uppercase tracking-wider">{{ __('Type') }}</th>
                            <th class="px-6 py-3 bg-gray-50 text-center text-base leading-4 font-medium text-gray-500 uppercase tracking-wider">{{ __('Sequence') }}</th>
                            <th class="px-6 py-3 bg-gray-50 text-center text-base leading-4 font-medium text-gray-500 uppercase tracking-wider">{{ __('Label') }}</th>
                            <th class="px-6 py-3 bg-gray-50 text-center text-base leading-4 font-medium text-gray-500 uppercase tracking-wider">{{ __('Url') }}</th>
                        ...
                        @if ($data->count())
                            @foreach ($data as $item)
                                <tr>
                                    <td class="px-6 py-2 text-center">{{ $item->type }}</td>
                                    <td class="px-6 py-2 text-center">{{ $item->sequence }}</td>
                                    <td class="px-6 py-2 text-center">{{ $item->label }}</td>
    ...
    <x-jet-dialog-modal wire:model="modalFormVisible">
        ...
        <x-slot name="content">
            <div class="mt-4">
                ...
                <x-jet-input wire:model="label" id="label" class="block mt-1 w-full" type="text"></x-jet-input>
            ...
            <div class="mt-4">
                ...
                <x-jet-input wire:model="sequence" id="sequence" class="block mt-1 w-full" type="text"></x-jet-input>
                @error('sequence') <span class="error">{{ $message }}</span> @enderror
            </div>
            <div class="mt-4">
                <x-jet-label for="sequence" value="{{ __('Type') }}"></x-jet-label>
                <label>
                    <select wire:model="type" class="block appearance-none w-full bg-gray-100 border border-gray-200 text-gray-700 py-3 px-4 pr-8 round leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                        ...
                        <option value="FooterNav">FooterNav</option>
                    </select>
                </label>
            </div>
        </x-slot>
